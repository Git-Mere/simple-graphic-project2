...
// else Desktop will pick the highest OpenGL version by default
    ...
    void Application::imguiSelectDemo()
    {
        ...
        if (ImGui::BeginMenu("Select Demo"))
        {
            ...
            supported_demos.push_back(NameType{ "Value Noise", Demos::ValueNoise });
            supported_demos.push_back(NameType{ "Gradient Noise", Demos::GradientNoise });
            limit_version = 400;
            if (!environment::opengl::IsOpenGL_ES || version >= limit_version)
            {
                supported_demos.push_back(NameType{ "Curve", Demos::Curve });
            }
