    ...
    for (const auto& attribute : buffer_layout)
    {
        IF_CAN_DO_OPENGL(4, 5)
        {
            ...
            GL::VertexArrayAttribFormat(
                vertex_array_handle, attribute.vertex_layout_location, attribute.component_dimension, attribute.component_type, attribute.normalized, attribute.relative_offset);
        ...
        else
        {
            ...
            GL::VertexAttribPointer(
                ...
                reinterpret_cast<const GLvoid*>(static_cast<GLsizeiptr>(attribute.relative_offset) + static_cast<GLsizeiptr>(attribute.offset)));
                //reinterpret_cast<const GLvoid*>(static_cast<GLsizeiptr>(attribute.relative_offset) + static_cast<GLsizeiptr>(attribute.offset)));
            Use(false);
...
void GLVertexArray::SetIndexBuffer(GLIndexBuffer&& the_indices)
{
    ...
    IF_CAN_DO_OPENGL(4, 5)
    {
        GL::VertexArrayElementBuffer(GetHandle(), the_indices.GetHandle());
    }
    else
    {
        Use();
...
{
    GL::DrawArrays(vertex_array.GetPrimitivePattern(), vertex_array.GetIndicesCount(), vertex_array.GetVertexCount());
}
