...
GLVertexBuffer::GLVertexBuffer(GLsizei size_in_bytes) : size(size_in_bytes)
{
    ...
    IF_CAN_DO_OPENGL(4, 5)
    {
        ...
        GL::NamedBufferStorage(buffer_handle, size_in_bytes, no_data, GL_DYNAMIC_STORAGE_BIT);
    }
    else
    {
...
GLVertexBuffer::GLVertexBuffer(GLVertexBuffer&& temp) noexcept : size(temp.size), buffer_handle(temp.buffer_handle)
{
