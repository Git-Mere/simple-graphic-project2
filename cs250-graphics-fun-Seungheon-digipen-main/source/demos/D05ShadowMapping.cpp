...
namespace demos
{
    ...
    void D05ShadowMapping::renderToDepthBuffer() const
    {
        ...
        if (drawBackFacesForRecordDepthPass == true)
        {
            drawSceneObjects(shaders[Shaders::WriteDepth], GL_FRONT);
        }
        else
        {
            drawSceneObjects(shaders[Shaders::WriteDepth], GL_BACK);
        }
        GL::CullFace(GL_BACK);
    ...
    {
        GL::ClearColor(FogColor.r, FogColor.g, FogColor.b, 1.f);
        GL::Clear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
        GL::Enable(GL_DEPTH_TEST);
        GL::DepthMask(GL_TRUE);
    ...
    void D05ShadowMapping::drawLightFrustum() const
    {
        if (shouldDrawLightFrustum && cameraMode != CameraMode::Light)
        {
            ...
            shaders[Shaders::Fill].SendUniform(Uniforms::Diffuse, glm::vec3(1.0));
